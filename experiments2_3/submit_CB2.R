...
load("./experiments2_3/sweepExp2.RData") #load sweep
...
for (x in 1:nrow(paramsweep))
    {
        result<-sim(mat=matrixGenerator(randomPoints(1000),k=paramsweep$k[x]),
                    z=paramsweep$z[x],mu=paramsweep$mu[x],sigma=paramsweep$sigma[x],
                    timeSteps=1000,verbose=FALSE,mode="CopyTheBest")
...
write.csv(paramsweep,file="./results/Exp2CopyTheBest.csv")
